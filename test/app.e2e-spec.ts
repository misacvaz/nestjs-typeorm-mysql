...
describe('AppController (e2e)', () => {
  ...
  it('Registrar um novo usuário', async () => {
    ...
    console.log(accessToken); // Verifique o valor do token
  });
  ...
  it('Registrar um novo usuário como administrador', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/register')
      .send({ ...authRegisterDTO, role: Role.Admin, email: 'verde@gmail.com' });
  ...
  it('Valicar se a funçáo do novo usuário ainda é User', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/me')
